...
(function () {
    ...
    const STALE_MS     = 90000; // entries older than 90s are considered dead tabs
    ...
    // ── Self-registration ─────────────────────────────────────────────────────
    // Each tab writes its own entry into a shared JSON array in GM storage.
    // Race-conditions are accepted: 30-second refresh interval means at worst
    // one refresh cycle separates a write collision from self-correction.
    ...
    // ── Close signaling ───────────────────────────────────────────────────────
    // When the generated viewer sends postMessage({savetabs:'CLOSE_ALL'}),
    // the triggering tab sets GM_setValue(KEY_CLOSE, timestamp).
    // All other tabs hear it via GM_addValueChangeListener and close themselves.
    ...
    // ── Main action ───────────────────────────────────────────────────────────
    function doSaveTabs() {
        registerSelf(); // refresh own entry before reading
